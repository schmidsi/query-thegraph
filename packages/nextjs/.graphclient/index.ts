...
const graphNetworkArbitrumTransforms = [];
const graphNetworkMainnetTransforms = [];
const graphNetworkArbitrumHandler = new GraphqlHandler({
              name: "graph-network-arbitrum",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-arbitrum"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("graph-network-arbitrum"),
              logger: logger.child("graph-network-arbitrum"),
              importFn,
            });
const graphNetworkMainnetHandler = new GraphqlHandler({
              name: "graph-network-mainnet",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-mainnet"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("graph-network-mainnet"),
              logger: logger.child("graph-network-mainnet"),
              importFn,
            });
sources[0] = {
          name: 'graph-network-arbitrum',
          handler: graphNetworkArbitrumHandler,
          transforms: graphNetworkArbitrumTransforms
        }
sources[1] = {
          name: 'graph-network-mainnet',
